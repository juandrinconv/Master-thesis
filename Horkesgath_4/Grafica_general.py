...
# Caudales a evaluar
Q_eval = np.array([1, 2, 3, 4, 8, 12, 16, 20, 24, 28, 30, 45, 60, 70, 72, 74, 76, 78, 80, 82, 84, 88, 92, 96, 97])

# 1) Funcion de ajuste (Exponencial)
coef_ajuste = data["Funcion de ajuste"]["coeficientes"]
def exp_model(Q, a, b):
    return a * np.exp(b * Q)
a, b = coef_ajuste
S_ajuste = exp_model(Q_eval, a, b)
...
# Puntos medidos
Q_meas = np.array([30, 45, 60, 70])
S_meas = np.array([3.88, 6.35, 10.26, 14.16])
...
plt.text(0.1, 0.2, string_datos_extrapolados_1,
...
plt.text(0.45, 0.2, string_datos_medidos,
         transform=plt.gca().transAxes,
         fontsize=12,
         verticalalignment='top',
         bbox=dict(boxstyle='round', facecolor='azure', alpha=0.8))
...
plt.text(0.72, 0.2, string_datos_extrapolados_2,
         transform=plt.gca().transAxes,
         fontsize=12,
         verticalalignment='top',
         bbox=dict(boxstyle='round', facecolor='azure', alpha=0.8))
