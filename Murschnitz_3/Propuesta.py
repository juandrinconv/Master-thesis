...
Q_LIST = np.array([7.23, 14.65, 28.83, 32.69])
S_LIST = np.array([1.65, 2.9, 10.2, 32.85])

# Modelo: S = L*Q + M*Q**2 + I*Q**n

def residuals(params: Sequence[float]) -> np.ndarray:
    L, M, I, n = params
    return S_LIST - (L * Q_LIST + M * Q_LIST**2 + I * Q_LIST**n)
...
if __name__ == '__main__':
    ...
    initial_guess = [0.0001, 0.0001, 0.0001, 7]
    lower_bounds = [-2, -2, -2, 5]
    upper_bounds = [2, 2, 2, 10]
    ...
    # Root
    root_initial = [0.0001, 0.0001, 0.0001, 12]
    ...
    if parametros_finales is not None:
        ...
        S_pred = Lf * Q_LIST + Mf * Q_LIST**2 + If * Q_LIST**nf
        errores_relativos = ((S_LIST - S_pred) / S_LIST).tolist()

        # Parámetros de bootstrapping
        n_boot = 10000 # número de réplicas
        sigma_S = np.std(S_LIST)  # Desviación estándar para el ruido en S_LIST
        ...
        for i in range(n_boot):
            S_sim = S_LIST + np.random.normal(0, sigma_S, size=S_LIST.shape)
            res_b = minimize(
                fun=lambda p: np.sum(np.abs(residuals_sub(p, Q_LIST, S_sim))),
